...
const ProjectBox: React.FC<ProjectBoxProps> = ({ strings, title, description, projectUrl, slidesPerView, spaceBetween, photoStyle, sx }) => {
    ...
    return (
      <Box sx={{width: isSmallScreen ? '100vw' : '40vw', bgcolor: 'secondary.main', padding: isSmallScreen ? '' : '1vw',  height: '100%', borderRadius: '10px', ...sx}}>
            ...
            <Box sx={{padding: isSmallScreen ? '5vw' : '1vw', display: 'flex', flexDirection: 'column', justifyContent: 'space-evenly', gap: isSmallScreen ? '3vw' : '1vw'}}>
              <Box>
                <Typography variant='h3' sx={{fontWeight: '800', width: isSmallScreen ? '100vw' : ''}}>{title}</Typography>
                <Typography sx={{fontWeight: '600', fontSize: isSmallScreen ? '5vw' : '1vw'}}>{description}</Typography>
              </Box>
                {
                  projectUrl &&
