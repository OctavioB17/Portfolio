    ...
    <Box sx={{width: '100vw', bgcolor: 'primary.main', padding: isSmallScreen ? '' : '5vw', paddingTop: isSmallScreen ? '15vw' : '3vw', paddingBottom: isSmallScreen ? '5vw' : '3vw', display: 'flex', flexDirection: 'column', gap: '3vw' }}>
      ...
      <Box sx={{display: 'flex'}}>
        <ImageList sx={{width: '100%', display: isSmallScreen ? 'flex' : '', flexDirection: isSmallScreen ? 'column' : '', alignItems: isSmallScreen ? 'center' : '', }} cols={isSmallScreen ? 1 : 2} gap={50}>
            <ProjectBox sx={{borderRadius: isSmallScreen ? 0 : '10px' }} photoStyle={{height: isSmallScreen ? '60vw' : '20vw', borderTop: '5px solid #f3d2c1', backgroundColor: 'secondary.main', position: 'relative', right: isSmallScreen ? '13vw' : ''}} title='Culzoni Digital' description='Culzoni ERP. Dedicated to manage sales, stock and billing from Mercado Libre. Culzoni ERP. Our all-in-one solution automates every step, from sales entry to billing, ensuring your business runs smoothly and efficiently.' strings={[CulzoniOne, CulzoniTwo, CulzoniThree, CulzoniFour, CulzoniFive]} projectUrl='/projects/culzoniDigital' slidesPerView={1} spaceBetween={50}/>
            <ProjectBox sx={{borderRadius: isSmallScreen ? 0 : '10px' }} photoStyle={{height: isSmallScreen ? '60vw' : '20vw', borderTop: '5px solid #f3d2c1', backgroundColor: 'secondary.main', position: 'relative', right: isSmallScreen ? '13vw' : ''}} title='McFly' description='All-in-one photography portfolio application where users can upload, edit, and manage all their photos. Advanced photo editing section. Adjust brightness, contrast, saturation, grayscale, rotate, crop images, and apply a variety of filters.' strings={[McFlyTwo, McFlyOne, McFlyThree, McFlyFour]} projectUrl='/projects/mcfly' slidesPerView={1} spaceBetween={50}/>
            <ProjectBox sx={{borderRadius: isSmallScreen ? 0 : '10px' }} photoStyle={{height: isSmallScreen ? '60vw' : '20vw', borderTop: '5px solid #f3d2c1', backgroundColor: 'secondary.main', position: 'relative', right: isSmallScreen ? '13vw' : ''}} title='E-Redux' description='Simple E-commerce using Redux power. Implemented Redux to manage global states, with a focus on cart functionality. You can see how I use Redux slicers to perform operations like adding and removing products, as well as updating the cart on page reloads.' strings={[EReduxTwo, EReduxOne, EReduxThree, EReduxFour]} projectUrl='/projects/eredux' slidesPerView={1} spaceBetween={50}/>
            <ProjectBox sx={{borderRadius: isSmallScreen ? 0 : '10px' }} photoStyle={{height: isSmallScreen ? '60vw' : '20vw', borderTop: '5px solid #f3d2c1', backgroundColor: 'secondary.main', position: 'relative', right: isSmallScreen ? '13vw' : ''}} title='Portfolio' description='This portfolio is a testament to my dedication and passion for web development. It has been created not only to highlight my skills and achievements but also to push the boundaries of my own capabilities. With a focus on interactive design and modern aesthetics, it stands as a representation of industry-leading standards.' strings={[PortfolioOne, PortfolioTwo, PortfolioThree]} projectUrl='/projects/portfolio' slidesPerView={1} spaceBetween={50}/>
        </ImageList>
      </Box>
      <Box sx={{width: '100%', display: 'flex', justifyContent: isSmallScreen ? 'start' : 'end', padding: '1vw'}}>
        <Button onClick={() => navigate('projects')} variant='outlined' sx={{color: 'button.textPrimary', textTransform: 'none', fontWeight: 600, fontSize: isSmallScreen ? '1.7rem' : '1vw'}}>View all my projects...</Button>
      </Box>
    </Box>
  )
}
